...
5. For every ready id printed: run `python3 run/next.py start <id>`, then set up its worktree:
   - **If `.worktrees/<id>` does not exist** (first attempt for this node): run `git worktree add .worktrees/<id> -b node/<id> main`. Then integrate each dependency's verified code, in the order the ids appear in that node's `deps` in `plan/manifest.json`: for each `<dep>`, run `git -C .worktrees/<id> merge --no-edit node/<dep>`.
     - If a merge fails (conflict): run `git -C .worktrees/<id> merge --abort`, then run `python3 run/next.py record <id> fail 'merge conflict integrating dep <dep> into worktree'`, do **not** launch a builder for this id on this attempt, and move on to the next ready id.
   - **If `.worktrees/<id>` already exists** (this is a retry): do **not** run `git worktree add` — the branch `node/<id>` already exists and re-creating it fails — and do **not** re-merge the deps. Reuse the worktree as-is: it still holds the previous failed attempt's files, which is exactly what lets the builder see and address the verifier's notes against the real code.
