---
description: Build a Launchpad-registered app as a graph of builder/verifier nodes.
---
...
This is the Launchpad-flavored twin of `/build-app`: same plan/build/verify graph, plus a setup phase that registers the app with PING Launchpad (Entra app registration, deploy identity, Dockerfile, CI workflow) before any node starts building.
...
Do **not** ask about ERP system, cloud provider, auth system, or deployment target — Launchpad already knows these (Infor M3, Azure, Entra ID, Innovation Kubernetes) and Phase 1.5 will supply them.
...
## Phase 1.5 — Launchpad Setup

Skip this phase if `plan/manifest.json` already exists, or if the argument is `resume`.

This phase registers the app with Launchpad and drops generated scaffolding into `main` before any builder worktree branches off it. It assumes the `lp-*` repo already exists and is checked out here.

1. Confirm the repo is set up: run `git remote get-url origin`. If it fails, or the remote name doesn't look like an `lp-*` repo, stop and tell the user to create/clone the Launchpad app repo first (same prerequisite `/build-app` has for the template repo), then re-run this command.
2. Call `plan_build` with the approved `plan/design.md` to get Launchpad's stack and deploy recommendation. Do not ask the user clarifying questions Launchpad's answer already covers.
3. Call `entra_create_app_registration` and `create_deploy_identity` for the app (the repo-first rule is already satisfied by step 1).
4. Call `generate_app_dockerfile` and `generate_github_workflow` for the app. When the workflow needs a deploy job runner, use:
   ```yaml
   runs-on:
     group: Azure-SII-Runner
   ```
   never `runs-on: [self-hosted, Azure-SII-Runner]`. If the design calls for a database, also call `generate_app_database`.
5. Fill in `.claude/skills/stack/SKILL.md` from `plan_build`'s stack recommendation (language, framework, directory conventions). If it doesn't specify a test command or lint command, ask the user for just those two — everything else comes from Launchpad's answer.
6. Commit the generated files (Dockerfile, workflow, any manifests, the filled-in `stack` skill) directly to `main`: `git add -A && git commit -m 'Launchpad setup: app registration, deploy identity, CI scaffolding'`, then `git push`. Every node's worktree branches from `main` in Phase 3, so this must land before Phase 2.
7. Print a short summary: app registration name, deploy identity, and the files added. No approval gate here — this step is reversible (it's a normal commit) and non-destructive. Continue straight to Phase 2.
...
The manifest describes app code only — no Launchpad-specific nodes. Phase 1.5 already committed the infra scaffolding to `main`, so every node's worktree inherits it automatically.
...
Once the user has merged everything, remind them that promoting the app to production (`request_prod_promotion`, `grant_prod_approval`) is a manual Launchpad step outside this command's scope.
